      ...
      memoriesList.forEach((m) => {
        ...
        div.innerHTML =
          ...
          (m.type
            ...
            : '') +
          (project
            ? '<span>' + (mrid ? mrid.iconSm : '') + esc(project) + '</span>'
            : '') +
          '</div>' +
    ...
    .then((memoriesList) => {
      ...
      groupOrder.forEach(function (project) {
        ...
        nameDiv.innerHTML =
          (mrid2 ? mrid2.iconMd : '') +
          esc(name) +
          (!isOther ? '<span class="col-path">' + esc(short) + '</span>' : '');
    ...
    .then((data) => {
      ...
      header.innerHTML =
        ...
        (data.conversation_id
          ? '<span>origin: <a href="/conversations/' +
      ...
      if (data.type === 'index') {
        ...
        note.style.cssText =
          'background:#fefce8;border:1px solid #ca8a04;border-radius:6px;padding:8px 12px;margin:8px 0;font-size:12px;color:#854d0e';
        note.textContent =
          'This is the memory index — the only file loaded into context by default. Claude uses the descriptions here to decide which memories to read in full, so keeping them accurate and specific is important.';
        mainEl.appendChild(note);
      } else {
        var indexPath = data.path.replace(/\/[^/]+$/, '/MEMORY.md');
        var indexId = btoa(indexPath)
          .replace(/\+/g, '-')
          .replace(/\//g, '_')
          .replace(/=+$/, '');
        ...
        backLink.style.cssText =
          'display:inline-block;margin:8px 0;font-size:12px;color:#854d0e;text-decoration:none';
