  ...
  if (!_depsCache[key]) {
    if (key === 'mermaid') {
      ...
      _depsCache[key] = loadScript('https://cdn.jsdelivr.net/npm/vega@5')
        .then(function () {
          return loadScript('https://cdn.jsdelivr.net/npm/vega-lite@5');
        })
        .then(function () {
          return loadScript('https://cdn.jsdelivr.net/npm/vega-embed@6');
        });
    } else if (key === 'geojson') {
      _depsCache[key] = Promise.all([
      ...
      _depsCache[key] = loadScript(
        'https://cdn.jsdelivr.net/npm/pako@2/dist/pako.min.js',
      );
    } else {
  ...
  renderer: {
    code: function (token) {
      ...
      return (
        ...
        '<div class="rich-rendered" data-rich-id="' +
        id +
        '" data-rich-lang="' +
        lang +
        '" data-rich-src="' +
        encodeURIComponent(token.text) +
        '"></div>' +
        '<pre class="rich-source" style="display:none"><code>' +
        escaped +
        '</code></pre>' +
        '</div>'
      );
...
export function renderRichBlocks() {
  document
    .querySelectorAll('.rich-rendered[data-rich-src]:not(.rich-done)')
    .forEach(function (el) {
      ...
      loadRichDeps(lang)
        .then(function () {
          if (lang === 'mermaid') {
            mermaid
              ...
              .catch(function () {
                el.innerHTML =
                  '<em style="color:#999">Failed to render diagram</em>';
              });
          } else if (lang === 'math') {
            try {
              ...
              el.innerHTML =
                '<em style="color:#999">Failed to render math</em>';
            }
            ...
            Viz.instance()
              .then(function (viz) {
                try {
                  ...
                  el.innerHTML =
                    '<em style="color:#999">Failed to render graph</em>';
                }
              })
              .catch(function () {
                el.innerHTML =
            ...
            try {
              ...
              vegaEmbed(el, spec, { actions: false }).catch(function () {
                el.innerHTML =
                  '<em style="color:#999">Failed to render chart</em>';
              });
            } catch (e) {
              el.innerHTML =
                '<em style="color:#999">Invalid Vega-Lite JSON</em>';
            }
            ...
            try {
              ...
              el.innerHTML =
                '<em style="color:#999">Failed to render notation</em>';
            }
            ...
            try {
              ...
              el.innerHTML =
                '<em style="color:#999">Failed to render mind map</em>';
            }
            ...
            img.onerror = function () {
              el.innerHTML =
                '<em style="color:#999">Failed to render PlantUML diagram</em>';
            };
            el.appendChild(img);
          }
        })
        .catch(function () {
          el.innerHTML =
            '<em style="color:#999">Failed to load rendering library</em>';
        });
    });
}
    ...
    .then((autolinks) => {
      ...
      const extensions = autolinks.map((al) => {
        ...
        return {
          ...
          level: 'inline',
          start: function (src) {
            return src.match(new RegExp(pre))?.index;
          },
...
export function renderMarkdown(text) {
  ...
  html = html.replace(
    /<command-message>([^<]*)<\/command-message>\s*<command-name>\/?([^<]+)<\/command-name>/g,
    function (match, msg, name) {
      if (!/^[\w][\w.-]*$/.test(name)) return match;
      return (
        '<a href="#" class="skill-link" data-skill="' +
        esc(name) +
        '" title="' +
        esc(msg) +
        '">/' +
...
function formatJson(str) {
  ...
  try {
    ...
    return pretty
      ...
      .replace(
        /"([^"\\]*(\\.[^"\\]*)*)"(\s*:)/g,
        '<span style="color:#881280">"$1"</span>$3',
      )
      .replace(
        /:\s*"([^"\\]*(\\.[^"\\]*)*)"/g,
        ': <span style="color:#1a7f37">"$1"</span>',
      )
    ...
    m.content.forEach(function (block) {
      ...
      if (block.name === 'TaskCreate' && block.result) {
        ...
        if (tm) {
          tasks[tm[1]] = {
            id: tm[1],
            subject: block.input.subject || tm[2],
            status: 'pending',
          };
        }
      }
        ...
        if (
          ...
          (block.result == null ||
            (typeof block.result === 'string' &&
              block.result.indexOf('Async agent launched') === 0))
        ) {
          running.push({
...
function buildStatusBar(messages) {
  ...
  if (taskState.counts.total > 0) {
    ...
    var taskIds = Object.keys(taskState.tasks).sort(function (a, b) {
      return a - b;
    });
    taskIds.forEach(function (id) {
      ...
      item.title =
        '#' + id + ' [' + t.status.replace('_', ' ') + '] ' + t.subject;
...
function renderToolInput(name, input) {
  ...
  if (name === 'Agent') {
    ...
    if (input.subagent_type)
      label +=
        ' <span class="agent-type">' + esc(input.subagent_type) + '</span>';
    var html = '<div class="agent-header">' + label + '</div>';
    if (input.description)
      html += '<div class="agent-desc">' + esc(input.description) + '</div>';
    if (input.prompt)
      html +=
        '<div class="agent-prompt">' + renderMarkdown(input.prompt) + '</div>';
    var extra = {};
    var hasExtra = false;
  ...
  if (name === 'ExitPlanMode') {
    ...
    if (input.plan) {
      html +=
        '<div class="plan-content">' + renderMarkdown(input.plan) + '</div>';
    }
    if (input.allowedPrompts && input.allowedPrompts.length) {
      html +=
        '<div class="plan-prompts"><div class="plan-prompts-label">Allowed prompts:</div>';
      input.allowedPrompts.forEach(function (p) {
        html +=
          '<div class="plan-prompt"><span class="plan-prompt-tool">' +
          esc(p.tool) +
          '</span> ' +
          esc(p.prompt) +
          '</div>';
    ...
    if (input.planFilePath) {
      ...
      html +=
        '<div class="plan-file"><a href="/plans/' +
        esc(planId) +
        '" style="color:#6d28d9">' +
        esc(input.planFilePath) +
        '</a></div>';
    }
  ...
  if (name === 'Write') {
    var header =
      '<div class="edit-file">' + esc(input.file_path || '') + '</div>';
    var content = input.content || '';
    var lines = content.split('\n').length;
    var label = lines + ' line' + (lines === 1 ? '' : 's');
    return (
      header +
      '<details class="write-content"><summary style="color:#888;font-size:11px;cursor:pointer">' +
      esc(label) +
      '</summary>' +
      '<pre style="margin:4px 0 0;background:#dfd;padding:6px 8px;border-radius:3px;max-height:300px;overflow-y:auto;white-space:pre-wrap">' +
      esc(content) +
      '</pre></details>'
    );
  }
  ...
  if (name === 'TaskCreate') {
    ...
    var tid = '';
    for (var k in taskSubjects) {
      if (taskSubjects[k] === input.subject) {
        tid = k;
        break;
      }
    }
  ...
  if (name === 'AskUserQuestion') {
    ...
    questions.forEach(function (q) {
      if (q.header)
        html +=
          '<div style="font-weight:600;color:#4338ca;margin-bottom:2px">' +
          esc(q.header) +
          '</div>';
      html +=
        '<div style="margin-bottom:4px">' + esc(q.question || '') + '</div>';
      if (q.options && q.options.length) {
        ...
        q.options.forEach(function (o) {
          html +=
            ...
            esc(o.label) +
            '</strong>' +
            (o.description ? ' &mdash; ' + esc(o.description) : '') +
            '</div>';
        });
        html += '</div>';
      }
  ...
  if (name === 'WebFetch') {
    ...
    main =
      '<a href="' +
      esc(input.url || '') +
      '" target="_blank" rel="noopener noreferrer" style="color:#4338ca">' +
      esc(input.url || '') +
      '</a>';
    if (input.prompt)
      main +=
  ...
  if (name === 'Workflow') {
    ...
    if (meta && meta.phases && meta.phases.length) {
      wfHtml +=
        '<ol style="margin:4px 0 2px 20px;padding:0;font-size:12px;color:#555">';
      meta.phases.forEach(function (p) {
        wfHtml +=
          '<li><strong>' +
          esc(p.title || '') +
          '</strong>' +
          (p.detail ? ' &mdash; ' + esc(p.detail) : '') +
          '</li>';
      });
      wfHtml += '</ol>';
    }
    if (script) {
      ...
      wfHtml +=
        '<details class="write-content"><summary style="color:#888;font-size:11px;cursor:pointer">script (' +
        lines +
        ' lines)</summary>' +
        '<pre style="margin:4px 0 0;background:#f5f5f5;padding:6px 8px;border-radius:3px;max-height:300px;overflow-y:auto;white-space:pre-wrap">' +
        esc(script) +
        '</pre></details>';
    }
    var wfExtra = {};
    ...
    if (hasWfExtra)
      wfHtml +=
        ...
        esc(JSON.stringify(wfExtra, null, 2)) +
        '</div>';
    return wfHtml;
  }
  if (main === null && name.indexOf('mcp__') === 0) {
    ...
    if (input.issueIdOrKey) {
      ...
      skip.push('summary');
      main =
        (input.projectKey ? esc(input.projectKey) + ': ' : '') +
        '<strong>' +
        esc(input.summary) +
        '</strong>';
  ...
  if (main === null)
    return (
      formatJson(JSON.stringify(input)) || esc(JSON.stringify(input, null, 2))
    );
  var extra = {};
  var hasExtra = false;
  for (var k in input) {
...
function blockToMarkdown(block) {
  ...
  if (block.type === 'tool_use') {
    if (block.name === 'Skill' && block.input && block.input.skill) {
      ...
      var extra = {};
      var hasExtra = false;
      for (var k in block.input) {
        if (k !== 'skill') {
          extra[k] = block.input[k];
          hasExtra = true;
        }
      }
      if (hasExtra)
...
function renderToolResult(block) {
  ...
  if (block.result) {
    ...
    if (useMarkdown) {
      html +=
        '<details class="agent-result"><summary>output <span class="expand-all" title="Expand/collapse all">&#x29C9;</span></summary>' +
        '<div class="agent-result-body">' +
        resultBody +
        '</div></details>';
    } else {
      html +=
        '<details class="tool-result-inline"><summary>output <span class="expand-all" title="Expand/collapse all">&#x29C9;</span></summary>' +
...
function isSkillTrigger(m) {
  ...
  if (
    m.role === 'user' &&
    m.content.some((b) => b.type === 'text' && /<command-name>/.test(b.text))
  )
    return true;
  if (
    m.role === 'assistant' &&
    m.content.some((b) => b.type === 'tool_use' && b.name === 'Skill')
  )
    return true;
  return false;
}
...
export function renderSkillPrompt(m) {
  ...
  return (
    '<details class="skill-prompt-details"><summary>Skill prompt</summary>' +
    '<div class="skill-prompt-body">' +
    body +
    '</div></details>'
  );
}
...
export function renderBlock(block) {
  ...
  if (block.type === 'thinking') {
    return (
      '<details class="block thinking"><summary>Thinking... <span class="expand-all" title="Expand/collapse all">&#x29C9;</span></summary>' +
  ...
  if (block.type === 'tool_use') {
    ...
    if (block.name === 'Agent' && block.input) {
      attrs =
        ' data-agent-desc="' +
        esc(block.input.description || block.input.subagent_type || 'Agent') +
        '"';
    }
    return (
      '<div class="block tool_use"' +
...
function renderSubagent(sub) {
  ...
  var html =
    ...
    esc(label) +
    ' <span class="expand-all" title="Expand/collapse all">&#x29C9;</span></summary>';
