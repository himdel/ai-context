...
export function forgeRepoUrl(f) {
  return f.base_url + '/' + f.repo;
}
export function forgeBranchUrl(f, branch) {
  if (f.type === 'gitlab')
    return forgeRepoUrl(f) + '/-/tree/' + encodeURIComponent(branch);
  if (f.type === 'gitea')
    return forgeRepoUrl(f) + '/src/branch/' + encodeURIComponent(branch);
  return forgeRepoUrl(f) + '/tree/' + encodeURIComponent(branch);
}
...
export function linkifyForge(html) {
  if (currentForgeRepo && currentForgeRepo.type !== 'gitlab') {
    html = html.replace(/(^|[>\s(])#(\d+)\b/g, function (match, before, id) {
      return (
        before +
        '<a href="' +
        esc(forgePrUrl(currentForgeRepo, id)) +
  ...
  html = html.replace(/<code>([0-9a-f]{7})<\/code>/g, function (match, sha) {
    var link =
      '<code><a href="#" class="git-show-link" data-sha="' +
      sha +
      '">' +
      sha +
      '</a></code>';
    if (currentForgeRepo) {
      link +=
        '<a href="' +
        esc(forgeCommitUrl(currentForgeRepo, sha)) +
        '" target="_blank" rel="noopener noreferrer" class="exticon" title="View on ' +
        forgeName(currentForgeRepo) +
        '">&#x2197;</a>';
    }
    return link;
  });
